...
int xmlMapperOpen_temp(MediaFile_t *media, FILE **xml)
{
    TRACE_INFO(MAPPR, BLD_GREEN "xmlMapperOpen_temp()" CLR_RESET);
    int retcode = FAILURE;
    ...
    if (media && media->container_mapper)
    {
        ...
        if (GetTempPath(MAX_PATH_SIZE, tempdir_mv) != 0)
        {
            snprintf(xmlMapPath, MAX_PATH_SIZE, "%s\\%s_temp.xml", tempdir_mv, media->file_name);
        }
#elif defined(__MINGW32__) || defined(__MINGW64__)
        char *tempdir = getenv("TEMP");
        if (tempdir)
        ...
        if (tempdir && strnlen(tempdir, MAX_PATH_SIZE) < MAX_PATH_SIZE)
        {
            snprintf(xmlMapPath, MAX_PATH_SIZE, "%s/%s_temp.xml", tempdir, media->file_name);
        }
        else
        {
            snprintf(xmlMapPath, MAX_PATH_SIZE, "/tmp/%s_temp.xml", media->file_name);
        }
#endif
        // File path fallback: put the xmlMap next to its media file
        if (strlen(xmlMapPath) == 0)
        {
            snprintf(xmlMapPath, MAX_PATH_SIZE, "%s\\%s_temp.xml", media->file_directory, media->file_name);
        }
